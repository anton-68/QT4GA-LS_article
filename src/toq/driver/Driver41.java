...
public class Driver41 {
	...
	static boolean PRINT_QUESTIONNAIRE = true;
	...
	public static void main(String[] args) throws FileNotFoundException {
		...
		Ankete a = new AnketeImpl("dat/" + subdirName + "/test0.dat");
		int noe = a.getNOE();
		int noq = a.getNOQ();
...
////////////////////////////////////////////////////
			...
			if (PRINT_INDIVIDUAL_RESULTS) {
				rs = new PrintStream(new File("res/" + subdirName + "/test0.res"));
				a.print(rs);
				rs.println("Mating rate: " + matingRate);
				rs.println("Mutation rate: " + mutationRate);
				rs.println("Size of population: " + pSize);
				rs.println("Length of chromosome: " + chLength);
				rs.println("No. of generation without improvement before stop: " + noGWOI);
				rs.println("Total max No. of generation: " + maxNOG);
			}
			...
			for (int j = 0; j < pfa1.length; j++) {
				...
				if (PRINT_QUESTIONNAIRE) {
					rs.println("Querstionnaire: ");
					s.getQuestionnaire().printSubtree(rs);
					rs.println();
				}				
				if (j == 0) {
			...
			if (PRINT_QUESTIONNAIRE) {
				rs.println("Querstionnaire: ");
				IndividualImpl ii = new IndividualImpl(p.getBestChromosome(), p);
				ii.print(a, rs);
				rs.println();
			}
